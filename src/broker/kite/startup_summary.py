...
def emit_startup_summary(provider) -> None:
    ...
    try:  # pragma: no cover
        ...
        if kp_mod is not None and not hasattr(kp_mod, '_KITE_PROVIDER_SUMMARY_EMITTED'):
    ...
    try:
        ...
        concise = int(bool(getattr(s, 'concise', True)))
        throttle_ms = int(getattr(s, 'kite_throttle_ms', 0) or 0)
        exp_fabrication = int(bool(getattr(s, 'allow_expiry_fabrication', True))) if hasattr(s, 'allow_expiry_fabrication') else 1
        cache_ttl = getattr(s, 'instruments_cache_ttl', None)
        retry_on_empty = int(bool(getattr(s, 'retry_on_empty', True))) if hasattr(s, 'retry_on_empty') else 1
        logger.info(
...
def _reset_provider_summary_state():  # test-only helper (mirrors startup summaries reset pattern)
    try:
        if '_KITE_PROVIDER_SUMMARY_EMITTED' in globals():
