...
os.environ['G6_CATALOG_HTTP_PORT'] = '9324'
# Force rebuild logic (though handler now always rebuilds)
...
for _ in range(60):
    try:
        with socket.create_connection(('127.0.0.1', 9324), timeout=0.2):
            break
    except Exception:
        time.sleep(0.05)
with urllib.request.urlopen('http://127.0.0.1:9324/catalog') as resp:
