...
def test_compute_expiry_status_synth():
    assert compute_expiry_status({'options':10,'synthetic_fallback':True}) == 'SYNTH'

def test_compute_expiry_status_ok():
    rec = {'options':10,'synthetic_fallback':False,'strike_coverage':0.9,'field_coverage':0.8}
    assert compute_expiry_status(rec) == 'OK'

def test_compute_expiry_status_partial_on_strike():
    rec = {'options':10,'synthetic_fallback':False,'strike_coverage':0.5,'field_coverage':0.9}
    assert compute_expiry_status(rec) == 'PARTIAL'

def test_compute_expiry_status_partial_on_field():
    rec = {'options':10,'synthetic_fallback':False,'strike_coverage':0.9,'field_coverage':0.4}
    assert compute_expiry_status(rec) == 'PARTIAL'
...
def test_aggregate_cycle_status_all_ok():
    assert aggregate_cycle_status([{'status':'OK'},{'status':'SYNTH'}]) == 'OK'
...
def test_env_override_invalid_values_ignored(monkeypatch):
    rec = {'options':10,'synthetic_fallback':False,'strike_coverage':0.74,'field_coverage':0.54}
